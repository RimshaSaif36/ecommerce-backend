...
const registerAdmin = asyncHandler(async (req, res) => {
  ...
  if (req.file) {
    try {
      const uploadResult = await S3UploadHelper.uploadFile(
        req.file,
        "admin-profiles"
      );
...
const deleteAdminProfileImage = asyncHandler(async (req, res) => {
  ...
  if (!admin) throw new ApiError(404, "Admin not found");
  if (!admin.adminProfileImage)
    throw new ApiError(400, "Admin does not have a profile image");
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, {}, "Admin profile image deleted successfully"));
});
// ------------------- GET ALL ADMINS -------------------
const getAllAdmins = asyncHandler(async (req, res) => {
  ...
  const adminsWithImages = await Promise.all(
    admins.map(async (admin) => {
      ...
      if (adminObj.adminProfileImage) {
        try {
          adminObj.adminProfileImageUrl = await S3UploadHelper.getSignedUrl(
            adminObj.adminProfileImage
          );
        } catch {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, adminsWithImages, "Admins retrieved successfully")
    );
});
